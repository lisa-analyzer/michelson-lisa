...
public class MichelsonLambdaVisitor extends MichelsonParserBaseVisitor<Object> {
	...
	public Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitLambdaMember(Lambda_instrContext lambda)  {
		
		paramType = visitType(lambda.type(0)).getValue();
		retType =  visitType(lambda.type(1)).getValue();

		Assignment assign = buildAssignment(new MichelsonUnknownData(cfg, MichelsonFileUtils.locationOf(filepath, lambda),"PARAMETER_VALUE" , paramType), paramType, true);
		...
		if (lambda.block() == null)
			return Triple.of(assign, nodeList, assign);
		Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visited = visitBlock(lambda.block());

		if(visited == null) {
			NoOp instrumented  = new NoOp(cfg, MichelsonFileUtils.locationOf(filepath, lambda));
			NodeList<CFG,Statement, Edge> nodeListBlock = new NodeList<>(SEQUENTIAL_SINGLETON);
			nodeListBlock.addNode(instrumented);
			visited = Triple.of(instrumented, nodeListBlock, instrumented);
		} else
		...
		returnTypeCheck(retType, symbolicStacks.get(currentStack).firstElement());
		
		return Triple.of(assign, nodeList, assign);
	}

	private void returnTypeCheck(Type retType, VariableRef firstElement) {
		if(firstElement.getStaticType() != Untyped.INSTANCE && retType.equals(firstElement.getStaticType()))
			throw new IllegalArgumentException("The return type of lambda do not match with the type on the top stack element");
	}
	...
	public Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitBlock(BlockContext ctx) {
		NodeList<CFG,Statement, Edge> nodes = new NodeList<>(SEQUENTIAL_SINGLETON);
		...
		for (int i = 0; i < ctx.data().size(); i++) {
			...
			if(st == null)
				continue;
			nodes.mergeWith(st.getMiddle());
			if (first == null)
				first = st.getLeft();
			if (last != null && !last.equals(st.getLeft()))
				addEdge(nodes, new SequentialEdge(last, st.getLeft()));
			last = st.getRight();
		}
		...
		return Triple.of(first, nodes, last);
	}
	...
	private Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitSomeData(Some_dataContext ctx) {
		...
		if (data != null) {
			...
			nodes.addNode(some_data);

			return Triple.of(some_data, nodes, some_data);
		}
	...
	private Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitRightData(Right_dataContext ctx) {
		...
		if (data != null) {
			...
			nodes.addNode(right_data);

			return Triple.of(right_data, nodes, right_data);
		}
	...
	private Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitLeftData(Left_dataContext ctx) {
		...
		if (data != null) {
			...
			nodes.addNode(left_data);

			return Triple.of(left_data, nodes, left_data);
		}
...
//-------------------------------INSTRUCTION------------------------------------------------
	...
	public Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitInstruction(
		...
		if (ctx.SWAP() != null)
			...
			e = visitCar(ctx.car_instr());
		else if (ctx.cdr_instr() != null)
			e = visitCdr(ctx.cdr_instr());
			...
			e = visitDup_instr(ctx.dup_instr());
		...
		if (StackProducer.class.isAssignableFrom((e.getClass()))) {
			...
			for( int i = 0; i < ((StackProducer) e).getNumberOfStackElementsProduced(); i++) { 
				...
				if(prev != null)
					addEdge(nodes, new SequentialEdge(prev, st));
				prev = st;
			}
			
			return Triple.of(start, nodes, end);
		}
		else {
			nodes.addNode(e);
			return Triple.of(e, nodes, e);
	...
	private VariableRef buildVariableRef(CodeLocation location, Type type, boolean push) {
		...
		VariableRef ref = new VariableRef(cfg, location,  "l"+idLambda+"_"+var_count, type);
		var_count++;
	...
	private Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitFail(CFG cfg, TerminalNode ctx) {
		...
		nodes.addNode(failWith);
		
		addEdge(nodes, new SequentialEdge(unit, failWith));;
		
		return Triple.of(unit, nodes, failWith);
	}
	...
	private Expression visitCar(Car_instrContext car_instrContext) {
		VariableRef elem = symbolicStacks.get(currentStack).pop();
		return new MichelsonCar(cfg, filepath, MichelsonFileUtils.getLine(car_instrContext), MichelsonFileUtils.getCol(car_instrContext), elem);
	}

	private Expression visitCdr(Cdr_instrContext cdr_instrContext) {
		VariableRef elem = symbolicStacks.get(currentStack).pop();
		return new MichelsonCdr(cfg, filepath, MichelsonFileUtils.getLine(cdr_instrContext), MichelsonFileUtils.getCol(cdr_instrContext), elem);
	}
	...
	public Expression visitDup_instr(Dup_instrContext ctx) {
		...
		if (ctx.NAL_CONST() == null) {
			...
			return new MichelsonDup(cfg, filepath, MichelsonFileUtils.getLine(ctx), MichelsonFileUtils.getCol(ctx), elem);
		}else {
			...
			return new MichelsonDup(cfg, filepath, MichelsonFileUtils.getLine(ctx), MichelsonFileUtils.getCol(ctx), nat, elem);
		}
	}
	...
	public Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitIf_none_instr(
		...
		if(then != null) {
			nodes.mergeWith(then.getMiddle());
			addEdge(nodes, new TrueEdge(ifGuard, then.getLeft()));
		}
		...
		if (ctx.block(1) != null && ctx.block(1) != null) {
			...
			if(otherwise != null) {
				nodes.mergeWith(otherwise.getMiddle());
				addEdge(nodes, new FalseEdge(ifGuard, otherwise.getLeft()));
			}
		...
		Pair<Statement, Statement> junctionAndEndNodes = computeJunctionAndEndNode(jp.getRight(), nodes, MichelsonFileUtils.locationOf(filepath, ctx));
		
		if (then != null)
			addEdge(nodes, new SequentialEdge(then.getRight(), junctionAndEndNodes.getLeft()));
		else
			addEdge(nodes, new TrueEdge(ifGuard, junctionAndEndNodes.getLeft()));
		
		if (otherwise != null)
			addEdge(nodes, new SequentialEdge(otherwise.getRight(), junctionAndEndNodes.getLeft()));
		else
			addEdge(nodes, new FalseEdge(ifGuard, junctionAndEndNodes.getLeft()));

		cfs.add(new IfThenElse(nodes, ifGuard, junctionAndEndNodes.getRight(), then != null ? then.getMiddle().getNodes() : Collections.emptyList(),
				otherwise == null ? Collections.emptyList() : otherwise.getMiddle().getNodes()));

		return Triple.of(ifGuard, nodes, junctionAndEndNodes.getRight());
	}
	...
	private Expression[] buildNExpressionArray(MichelsonNaturalData nat) {
		Expression[] exprs = new Expression[nat.getValue().intValue()+1];
		exprs[0] = nat;
	...
	public Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitUnpair_instr(Unpair_instrContext ctx) {
		...
		if (ctx.NAL_CONST() == null)
			...
			e = new MichelsonUnpair(cfg, filepath, MichelsonFileUtils.getLine(ctx), MichelsonFileUtils.getCol(ctx),
					buildNExpressionArray((MichelsonNaturalData) visitNatConst(ctx.NAL_CONST())));
		}
		
		NodeList<CFG,Statement, Edge> nodes = new NodeList<>(SEQUENTIAL_SINGLETON);
		
		VariableRef var = symbolicStacks.get(currentStack).pop();
		Assignment left = buildAssignment(new LeftUNPAIR(cfg, MichelsonFileUtils.locationOf(filepath, ctx), var));
		Assignment right = buildAssignment(new RightUNPAIR(cfg, MichelsonFileUtils.locationOf(filepath, ctx), var));
		
		symbolicStacks.get(currentStack).push((VariableRef) left.getLeft());
		symbolicStacks.get(currentStack).push((VariableRef) right.getLeft());
		...
		addEdge(nodes, new SequentialEdge(e, left));
		addEdge(nodes, new SequentialEdge(left, right));

		return Triple.of(e, nodes, right);
	}
	...
	public Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitIf_left_instr(
		...
		nodes.addNode(getLeft);
		addEdge(nodes, new TrueEdge(ifGuard, getLeft));
		
		Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> then = visitBlock(ctx.block(0));
		
		if(then != null) {
			nodes.mergeWith(then.getMiddle());
			addEdge(nodes, new SequentialEdge(getLeft, then.getLeft()));
		}
		...
		nodes.addNode(getRight);
		addEdge(nodes, new FalseEdge(ifGuard, getRight));
		...
		if (ctx.block(1) != null && ctx.block(1) != null) {
			...
			if(otherwise != null) {
				nodes.mergeWith(otherwise.getMiddle());
				addEdge(nodes, new SequentialEdge(getRight, otherwise.getLeft()));
			}
		}
		...
		if (then != null)
			...
			addEdge(nodes, new TrueEdge(getLeft, junctionAndEndNodes.getLeft()));
		
		if (otherwise != null)
			addEdge(nodes, new SequentialEdge(otherwise.getRight(), junctionAndEndNodes.getLeft()));
		else
			addEdge(nodes, new FalseEdge(getRight, junctionAndEndNodes.getLeft()));
		...
		cfs.add(new IfThenElse(nodes, ifGuard, junctionAndEndNodes.getRight(), thenNodes, otherwiseNodes));

		return Triple.of(ifGuard, nodes, junctionAndEndNodes.getRight());
	}

	private Pair<Statement, Statement> computeJunctionAndEndNode(List<Assignment> phiList,
			NodeList<CFG,Statement, Edge> nodes, SourceCodeLocation location) {
		...
		if(!phiList.isEmpty()) {
			...
			phiList.forEach(n -> nodes.addNode(n));
			
			for(int i=0; i < phiList.size()-1; i++)
				addEdge(nodes, new SequentialEdge(phiList.get(i), phiList.get(i+1)));

			junctionNode = phiList.get(0);
			endNode = new NoOp(cfg, location);
			nodes.addNode(endNode);
			nodes.addEdge(new SequentialEdge(phiList.get(phiList.size()-1), endNode));
		} else {
			endNode = new NoOp(cfg, location);
	...
	public Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitIf_cons_instr(
		...
		nodes.addNode(firstValueOfList);
		
		Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> then = visitBlock(ctx.block(0));
		
		if(then != null) {
			nodes.mergeWith(then.getMiddle());
			addEdge(nodes, new SequentialEdge(firstValueOfList, then.getLeft()));
			
		}

		addEdge(nodes, new TrueEdge(listGuard, firstValueOfList));
		...
		if (ctx.block(1) != null && ctx.block(1) != null) {
			...
			if(otherwise != null) {
				nodes.mergeWith(otherwise.getMiddle());
				addEdge(nodes, new FalseEdge(listGuard, otherwise.getLeft()));
			}
		...
		if (then != null)
			...
			addEdge(nodes, new TrueEdge(listGuard, junctionAndEndNodes.getLeft()));
		
		if (otherwise != null)
			addEdge(nodes, new SequentialEdge(otherwise.getRight(), junctionAndEndNodes.getLeft()));
		else
			addEdge(nodes, new FalseEdge(listGuard, junctionAndEndNodes.getLeft()));

		cfs.add(new IfThenElse(nodes, listGuard, junctionAndEndNodes.getRight(), then != null ? then.getMiddle().getNodes() : Collections.emptyList(),
				otherwise == null ? Collections.emptyList() : otherwise.getMiddle().getNodes()));

		return Triple.of(listGuard, nodes, junctionAndEndNodes.getRight());	
	}
	...
	public Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitMap_instr(Map_instrContext ctx) {
		...
		if(block != null)
			nodes.mergeWith(block.getMiddle());
		...
		if(!phiList.isEmpty()) {
			...
			nodes.addNode(endNode);
			nodes.addNode(firstElement);
			addEdge(nodes, new SequentialEdge(firstElement, endNode));
		}
			
		if(block != null) {
			nodes.addNode(block.getLeft());
			addEdge(nodes, new SequentialEdge(iterGuard, block.getLeft()));
			addEdge(nodes, new SequentialEdge(block.getRight(), iterGuard));
		}else
			addEdge(nodes, new SequentialEdge(iterGuard, iterGuard));
		
		addEdge(nodes, new SequentialEdge(iterGuard, junctionNode));
		
		cfs.add(new Loop(nodes, iterGuard, endNode, block != null ? block.getMiddle().getNodes() : Collections.emptyList()));

		return Triple.of(iterGuard, nodes, endNode);
	}
	...
	public Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitIf_instr(If_instrContext ctx) {
		...
		if(then != null) {
			nodes.mergeWith(then.getMiddle());
			addEdge(nodes, new TrueEdge(booleanGuard, then.getLeft()));
		}
		...
		if (ctx.block(1) != null && ctx.block(1) != null) {
			...
			if(otherwise != null) {
				nodes.mergeWith(otherwise.getMiddle());
				addEdge(nodes, new FalseEdge(booleanGuard, otherwise.getLeft()));
			}
		}
		...
		if(then != null) {
			...
			addEdge(nodes, new TrueEdge(booleanGuard, junctionAndEndNodes.getLeft()));
		
		if (otherwise != null)
			addEdge(nodes, new SequentialEdge(otherwise.getRight(), junctionAndEndNodes.getLeft()));
		else
			addEdge(nodes, new FalseEdge(booleanGuard, junctionAndEndNodes.getLeft()));

		cfs.add(new IfThenElse(nodes, booleanGuard, junctionAndEndNodes.getRight(), then != null ? then.getMiddle().getNodes() : Collections.emptyList(),
				otherwise == null ? Collections.emptyList() : otherwise.getMiddle().getNodes()));

		return Triple.of(booleanGuard, nodes, junctionAndEndNodes.getRight());
	}
	...
	public Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> visitLoop_instr(Loop_instrContext ctx) {
		...
		nodes.addNode(assignBooleanGuard);
		...
		if(!phiList.isEmpty()) {
			...
			junctionNode = endNode;
			nodes.addNode(endNode);
			nodes.addNode(mapAssignamentNode.get(firstElement));
			addEdge(nodes, new SequentialEdge(mapAssignamentNode.get(firstElement), endNode));
		}
			
		if(block != null) {
			nodes.addNode(block.getLeft());
			addEdge(nodes, new SequentialEdge(booleanGuard, block.getLeft()));
			addEdge(nodes, new SequentialEdge(block.getRight(), booleanGuard));
		}else
			addEdge(nodes, new SequentialEdge(booleanGuard, booleanGuard));
		
		addEdge(nodes, new SequentialEdge(booleanGuard, junctionNode));
		
		cfs.add(new Loop(nodes, booleanGuard, endNode, block != null ? block.getMiddle().getNodes() : Collections.emptyList()));

		return Triple.of(booleanGuard, nodes, endNode);
	...
	public Expression visitLambda_instr(
		...
		currentStack++;
		symbolicStacks.add(new MichelsonStack<VariableRef>());
		
		symbolicStacks.get(currentStack).push(new VariableRef(cfg, MichelsonFileUtils.locationOf(filepath, ctx), "lambdaParam"));
		
		MichelsonTypeExpression parameterType = visitType(ctx.type(0));
		MichelsonTypeExpression returnType = visitType(ctx.type(1));
		
		Triple<Statement, NodeList<CFG,Statement, Edge>, Statement> block = null;
		//TODO: I should create a new cfg to build the block of a lambda apart
		...
		//	NodeList<CFG,Statement, Edge> nodeListBlock = new NodeList<>(SEQUENTIAL_SINGLETON);
		//	nodeListBlock.addNode(instrumented);
		//	block = Triple.of(instrumented, nodeListBlock, instrumented);
		//}
		
		MichelsonLambda lambda = new MichelsonLambda(cfg, filepath, MichelsonFileUtils.getLine(ctx), MichelsonFileUtils.getCol(ctx), parameterType.getValue(), returnType.getValue());
		mapLambda.put(lambda, block);

		currentStack++;
		symbolicStacks.add(MichelsonStack.copy(symbolicStacks.get(beforeStack))); //restore stack
